...
import {Alert, Badge, Button, Card, Descriptions, Tag} from "antd";
...
Не закрывайте и не обновляйте эту страницу. Статус заказа обновляется автоматически, его Вы можете посмотреть в таблице ниже." type="error" />
      <Descriptions className="mt-5 mb-5" bordered>
        ...
        {dataForDownload?.length && (<Descriptions.Item label="Файл для скачивания:" span={3}>
          <ExportToExcel apiData={dataForDownload}/>
        </Descriptions.Item>)}
        {!dataForDownload?.length && (<Descriptions.Item label="Результат:" span={3}>
          <Tag color="red">
            Ничего не найдено в выбранной области поиска. <br/>
